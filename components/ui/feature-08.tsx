...
/* ══════════════════════════════════════════════════════════════════════
   ...
   1. LES JETONS shadcn (`bg-background`, `bg-border`, `text-warm`,
      `--warm-glow`) n'existent pas : filets `--o-line`, fond `#0d0d10`,
      projecteur blanc à 6 % — la charte est monochrome, le « warm » de
      Hirael n'a pas d'équivalent et n'en aura pas.
...
function Carte({ c }: { c: CaseArpentee }) {
  ...
  return (
    <div
      ...
      className="group relative flex h-full flex-col justify-start gap-5 bg-[#0d0d10] px-6 pb-6 pt-7 sm:gap-6 sm:px-7 sm:pb-7 sm:pt-8"
    >
      <div
        aria-hidden
        ...
        style={{
          background:
            "radial-gradient(circle 200px at var(--mx, 50%) var(--my, 50%), rgba(255,255,255,0.07), transparent 70%)",
        }}
      />
      ...
      <div className="relative z-10 flex w-fit items-center justify-center rounded-md border border-[var(--o-line)] bg-white/[0.04] p-3 text-[var(--o-text)] transition-colors duration-300 group-hover:border-[#52525b] [&>svg]:h-5 [&>svg]:w-5">
        {c.icone}
      </div>
