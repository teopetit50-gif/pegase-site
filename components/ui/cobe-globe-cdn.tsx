...
/* ══════════════════════════════════════════════════════════════════════
   GlobeCdn — globe WebGL interactif (cobe), repris de 21st.dev le
   16/09/2026 à la demande de Teo pour /offres.

   Le composant est recopié tel qu'il a été fourni. DEUX ajouts, et
   seulement deux, imposés par les règles du site :

   · `libelles` et `trafic` (vrais par défaut, donc le composant se
     comporte exactement comme la fiche d'origine). Mis à `false`, ils
     éteignent les pastilles de région et les compteurs « k req/s ».
     Pourquoi : les valeurs par défaut de la fiche sont les codes de
     région d'un hébergeur (iad1, sfo1, cdg1…) et des débits inventés.
     Une page publique du site ne nomme jamais nos outils
     (references/regles-maison.md) et n'affiche aucun chiffre de
     traction inventé. Sur /offres, les deux sont donc à `false` et le
     globe ne porte que des points.

   · les libellés se posent en `position-anchor`, une propriété que seul
     Chromium sait rendre aujourd'hui. Ailleurs ils restent à
     `opacity: 0` — ce n'est pas un bug, c'est le comportement de la
     fiche d'origine.

   ⚠ Ce composant peint en WebGL. Les sondes CDP du harnais de recette
   se figent dessus si le contexte ne s'initialise pas
   ([[webgl-gele-les-captures-cdp]]) : mesurer la page avec
   `--disable-webgl` et lire le globe à l'œil, pas à la sonde.
   ══════════════════════════════════════════════════════════════════════ */
...
interface GlobeCdnProps {
  ...
  className?: string;
  speed?: number;
  /** pastilles de région au-dessus des points (défaut : true) */
  libelles?: boolean;
  /** compteurs de débit au-dessus des arcs (défaut : true) */
  trafic?: boolean;
}
      ...
      {libelles &&
        markers.map((m) => (
          <div
            ...
            style={{
              position: "absolute",
              /* `positionAnchor` est typé par le React de ce dépôt : la
                 fiche d'origine le faisait précéder d'un @ts-expect-error,
                 que TypeScript signale ici comme inutile. */
