"use client";
/* ══════════════════════════════════════════════════════════════════════
   Lorani (agences d'architecture) — bouton-etoile.tsx

   COPIÉ le 24/09/2026 à 14 h 57 de `OMEGA/dossieros-site/src/components/
   ui/star-button.tsx` (port du bouton étoilé de la référence Parlo : une
   lueur qui parcourt le contour par `offset-path`, un liseré à motif
   pointillé, un texte en dégradé). Ce qui change :

   · UN LIEN, PAS UN BOUTON. La source rendait un <button> qui faisait
     `window.location.assign(href)` : un rechargement complet du site pour
     aller sur une page interne. Ici c'est `Lien` (un <Link> Next pour une
     adresse interne) : même dessin, navigation client, et il se lit comme
     un lien au clavier et aux lecteurs d'écran.

   · MONDE BLANC. Les `dark:` sont retirés ; reste le thème CLAIR de la
     référence : face blanche (le motif est « rempli » en `text-white`, les
     pointillés sont des TROUS dans ce remplissage), filet `border-black/10`,
     texte en dégradé du noir au neutral-400. Mais dans ce thème-là la lueur
     restait #FAFAFA : du blanc qui tourne sous une face blanche, invisible —
     l'effet disparaissait. Le sens s'inverse, pas la valeur : la lueur
     devient d'ENCRE (#404040) et se voit à travers le filet et les
     pointillés, comme la lueur blanche sur le noir. La flèche, blanche sur
     la face noire, passe à l'encre : blanche sur blanc, elle disparaissait.

   · `animate-star-btn` (keyframes de la source) devient la classe
     `architectes-star-btn` d'architectes.css.
   ══════════════════════════════════════════════════════════════════════ */
import React, { useEffect, useId, useRef } from "react";
import Lien from "@/components/Lien";
import { cn } from "@/lib/cn";

export interface StarButtonProps extends Omit<React.AnchorHTMLAttributes<HTMLAnchorElement>, "href"> {
  href: string;
  duration?: number;
  lightWidth?: number;
  lightColor?: string;
...
export default function StarButton({
  ...
  /* Le chemin que suit la lueur : le contour du bouton, mesuré une fois monté.
     Il est posé sur l'enveloppe intérieure : `Lien` ne transmet pas de ref. */
  useEffect(() => {
    const b = ref.current;
    if (b) b.style.setProperty("--path", `path('M 0 0 H ${b.offsetWidth} V ${b.offsetHeight} H 0 V 0')`);
  }, []);
  return (
    <Lien
      href={href}
    ...
    >
      <span
        ...
        className="relative inline-flex h-full w-full items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-[inherit] px-4 py-2"
      >
        <span
          aria-hidden="true"
          className="architectes-star-btn absolute inset-0 aspect-square bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)]"
          style={{ offsetPath: "var(--path)", offsetDistance: "0%", width: "var(--light-width)" }}
        />
        <span
          aria-hidden="true"
        ...
        >
          ...
          >
            <g clipPath={`url(#${uid}-clip0_408_119)`}>
              <path
            ...
            <defs>
              <clipPath id={`${uid}-clip0_408_119`}>
                <rect width="100" height="40" fill="white"></rect>
              </clipPath>
            </defs>
          </svg>
        </span>
        <span className="relative z-10 inline-flex items-center gap-1.5 bg-gradient-to-t from-black to-neutral-400 bg-clip-text text-transparent">
          <span>{children}</span>
          <svg
