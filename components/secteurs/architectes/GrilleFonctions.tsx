/* ══════════════════════════════════════════════════════════════════════
   Lorani (agences d'architecture) — GrilleFonctions.tsx

   COPIÉ le 24/09/2026 à 14 h 57 de `OMEGA/dossieros-site/src/components/
   fonctionnalites.tsx`. Ce qu'en disait la source :

   Port de « Features 8 » (Tailark, @meschacirung sur 21st.dev), posé le
   24/09 à la place des trois cartes quadrillées de la référence (Teo :
   « j'aime pas ce component, trouves-en un autre sur 21st.dev »). Grille de
   six colonnes, cinq cartes : un grand « 100 % » cerné d'une orbite, une
   empreinte que balaie un trait, une courbe, puis deux cartes larges
   (fenêtre de courbe, colonne d'interlocuteurs). Les tracés SVG sont ceux
   du composant, tels quels. Écarts assumés : nos textes ; les libellés du
   petit graphique en texte français ; des icônes lucide choisies pour nos
   fonctions ; nos tuiles d'application à la place des photos de visages
   (règle maison : aucun visage inventé) ; titres de carte en h3.

   CE QUE LE RAPATRIEMENT CHANGE :
   · Les `dark:` (17) sont retirés : Features 8 a un thème clair, c'est lui
     qui reste (orbite `text-muted` → #f5f5f5, empreinte zinc-400, titre
     « Alertes » zinc-800, cartes blanches `shadow-sm`).
   · L'ACCENT. La source avait mis l'accent du composant en BLANC (le trait
     qui balaie l'empreinte, les deux courbes, leur dégradé à 15 %, les
     repères du petit graphique, le chiffre « 55 % ») : sur le blanc, rien.
     Il passe à l'encre #0a0a0a, dégradés compris (`text-white/15` →
     `text-[#0a0a0a]/15`).
   · `before:bg-[#e5e5e5]` lisait un jeton du thème source, absent
     ici → `before:bg-[#e5e5e5]`. `rounded-tl-(--radius)` lit `--radius`,
     reposé sous `.p-architectes` (architectes.css).
   · `rounded-lg` / `rounded-md` → 10 / 8 px (rayons de la source).
   · REPORT du 24/09, 17 h 00 (seconde copie de la source) : une autre
     ...
     (Teo : « ajoute tout sur les sites »). Ici : deux cartes réécrites
     (« Une question suivie jusqu'à la réponse », et la date butoir des
     visas calée sur le délai de commande). Report par fusion à trois voies.
   ══════════════════════════════════════════════════════════════════════ */
...
const Carte = ({ className, children }: { className?: string; children: ReactNode }) => (
  <div className={cn("rounded-[10px] border bg-[#ffffff] text-[#0a0a0a] shadow-sm", className)}>{children}</div>
);
const Contenu = ({ className, children }: { className?: string; children: ReactNode }) => (
  <div className={cn("p-6 pt-0", className)}>{children}</div>
);
const Anneau = ({ children }: { children: ReactNode }) => (
...
const Pastille = ({ children }: { children: ReactNode }) => (
  <span className="block h-fit rounded border px-2 py-1 text-xs shadow-sm">{children}</span>
);
...
export default function GrilleFonctions() {
  return (
    <div className="relative mt-8 md:mt-16">
      <div className="relative z-10 grid grid-cols-6 gap-3">
        <Carte className="relative col-span-full flex overflow-hidden lg:col-span-2">
          <Contenu className="relative m-auto size-fit pt-6">
            <div className="relative flex h-24 w-56 items-center">
              <svg
                className="text-[#f5f5f5] absolute inset-0 size-full"
                viewBox="0 0 254 104"
                fill="none"
                aria-hidden="true"
              >
                <path d={ORBITE} fill="currentColor" />
              </svg>
              <span className="mx-auto block w-fit text-5xl font-semibold">100 %</span>
            </div>
            <h3 className="mt-6 text-center text-3xl font-semibold">Planches relues</h3>
          </Contenu>
        </Carte>

        <Carte className="relative col-span-full overflow-hidden sm:col-span-3 lg:col-span-2">
          <Contenu className="pt-6">
            <div className="relative mx-auto flex aspect-square size-32 rounded-full border before:absolute before:-inset-2 before:rounded-full before:border">
              <svg className="m-auto h-fit w-24" viewBox="0 0 212 143" fill="none" aria-hidden="true">
                <path className="text-zinc-400" d={EMPREINTE} fill="currentColor" />
                <g clipPath="url(#f8-empreinte-coupe)">
                  <path d={EMPREINTE} fill="url(#f8-empreinte-degrade)" />
                </g>
                <path
                ...
                <defs>
                  ...
                  >
                    ...
                    <stop className="text-[#0a0a0a]" offset="1" stopColor="currentColor" />
                  </linearGradient>
                  <clipPath id="f8-empreinte-coupe">
            ...
            <div className="relative z-10 mt-6 space-y-2 text-center">
              <h3 className="text-lg font-medium transition">Un lecteur par pièce</h3>
              <p className="text-[#737373]">
                Chaque type de pièce a sa propre lecture : un plan, une DPGF et une fiche technique ne se contrôlent pas
                de la même façon.
              </p>
            </div>
          </Contenu>
        ...
        <Carte className="relative col-span-full overflow-hidden sm:col-span-3 lg:col-span-2">
          <Contenu className="pt-6">
            <div className="pt-6 lg:px-6">
              <svg className=" w-full" viewBox="0 0 386 123" fill="none" aria-hidden="true">
                <rect width="386" height="123" rx="10" />
                <g clipPath="url(#f8-situation-coupe)">
                  <circle className="text-[#737373]" cx="29" cy="29" r="15" fill="currentColor" />
                  <path d="M29 23V35" stroke="#0a0a0a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  <path
                    d="M23 29L29 23L35 29"
                    stroke="#0a0a0a"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <text x="53" y="25" fill="currentColor" fontSize="10.5">
                    Lot 03 · situation n° 4
                  </text>
                  <text className="text-[#0a0a0a]" x="53" y="40" fill="currentColor" fontSize="13" fontWeight="600">
                    Écart de 7 %
                  </text>
                </g>
                <path fillRule="evenodd" clipRule="evenodd" d={AIRE} fill="url(#f8-situation-degrade)" />
                <path className="text-[#0a0a0a]" d={COURBE} stroke="currentColor" strokeWidth="3" />
                <defs>
                  <linearGradient
                    id="f8-situation-degrade"
                    x1="3"
                    y1="60"
                    x2="3"
                    y2="123"
            ...
            <div className="relative z-10 mt-14 space-y-2 text-center">
              <h3 className="text-lg font-medium transition">Situations suivies</h3>
              <p className="text-[#737373]">
                La situation reçue est comparée au marché et à la précédente, puis l&apos;écart est chiffré.
              </p>
            </div>
          </Contenu>
        </Carte>

        <Carte className="relative col-span-full overflow-hidden lg:col-span-3">
          <Contenu className="grid pt-6 sm:grid-cols-2">
            <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
              <Anneau>
                <Bell className="m-auto size-5" strokeWidth={1} />
              </Anneau>
              <div className="space-y-2">
                <h3 className="text-lg font-medium text-zinc-800 transition">Alertes à réception</h3>
                <p className="text-[#737373]">
                  Chaque situation, indice ou offre est lu dès sa réception, et l&apos;écart est signalé le jour même.
                  La date butoir de chaque visa est calée sur le délai de commande de l&apos;ouvrage.
                </p>
              </div>
            </div>
            <div className="rounded-tl-(--radius) relative -mb-6 -mr-6 mt-6 h-fit border-l border-t p-6 py-6 sm:ml-6">
              ...
              <svg className="w-full sm:w-[150%]" viewBox="0 0 366 231" fill="none" aria-hidden="true">
                <path fillRule="evenodd" clipRule="evenodd" d={AIRE_LARGE} fill="url(#f8-alertes-degrade)" />
                <path className="text-[#0a0a0a]" d={COURBE_LARGE} stroke="currentColor" strokeWidth="2" />
                <defs>
                  <linearGradient
                    id="f8-alertes-degrade"
                    x1="0.85108"
                    y1="0.947876"
                    x2="0.85108"
                    y2="230.114"
        ...
        <Carte className="relative col-span-full overflow-hidden lg:col-span-3">
          <Contenu className="grid h-full pt-6 sm:grid-cols-2">
            <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
              <Anneau>
                <MessageSquareText className="m-auto size-6" strokeWidth={1} />
              </Anneau>
              <div className="space-y-2">
                <h3 className="text-lg font-medium transition">Une question suivie jusqu&apos;à la réponse</h3>
                <p className="text-[#737373]">
                  Chaque point part en question à l&apos;entreprise ou au BET, avec l&apos;extrait joint. Lorani suit
                  ensuite la réponse, et la question qui attend depuis huit jours remonte le matin.
                </p>
              </div>
            </div>
            <div className="before:bg-[#e5e5e5] relative mt-6 before:absolute before:inset-0 before:mx-auto before:w-px sm:-my-6 sm:-mr-6">
              <div className="relative flex h-full flex-col justify-center space-y-6 py-6">
                <div className="relative flex w-[calc(50%+0.875rem)] items-center justify-end gap-2">
                  <Pastille>Entreprise · lot 03</Pastille>
                  <div className="ring-[#ffffff] size-7 rounded-[8px] ring-4">
                    <img className="size-full rounded-[8px]" src="/secteurs-architectes/icones/offres.svg" alt="" />
                  </div>
                </div>
                <div className="relative ml-[calc(50%-1rem)] flex items-center gap-2">
                  <div className="ring-[#ffffff] size-8 rounded-[8px] ring-4">
                    <img className="size-full rounded-[8px]" src="/secteurs-architectes/icones/plans.svg" alt="" />
                  </div>
                  <Pastille>BET structure</Pastille>
                </div>
                <div className="relative flex w-[calc(50%+0.875rem)] items-center justify-end gap-2">
                  <Pastille>Bureau de contrôle</Pastille>
                  <div className="ring-[#ffffff] size-7 rounded-[8px] ring-4">
                    <img className="size-full rounded-[8px]" src="/secteurs-architectes/icones/visa.svg" alt="" />
                  </div>
                </div>
              </div>
            </div>
