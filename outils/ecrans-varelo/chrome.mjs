...
export async function ouvrirSession({ largeur = 1440, hauteur = 900,
  ...
  await envoyer('Emulation.setDeviceMetricsOverride',
    { width: largeur, height: hauteur, deviceScaleFactor: 2, mobile: largeur < 768 });
  ...
  /* Le WebSocket global de Node ne retient pas la boucle d'événements : si le Mac est chargé et qu'une capture met
     plus de quelques secondes, Node sort en « unsettled top-level await » au lieu d'attendre. Une minuterie la retient. */
  const garde = setInterval(() => {}, 1000);
  const fermer = () => { clearInterval(garde); try { ws.close(); } catch {} proc.kill(); };
